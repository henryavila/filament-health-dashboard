{{-- Filament Health Dashboard — generic status grid + per-check meta drill-down.
     v1 scaffold: rich per-check infolist + remediation actions (CheckIntegration)
     are wired here in the UI pass. --}}
<x-filament-panels::page>
    @php
        ...
        $statusMeta = [
            'ok' => ['color' => 'success', 'icon' => 'heroicon-m-check-circle', 'label' => 'OK'],
            'warning' => ['color' => 'warning', 'icon' => 'heroicon-m-exclamation-triangle', 'label' => 'Atenção'],
            'failed' => ['color' => 'danger', 'icon' => 'heroicon-m-x-circle', 'label' => 'Falhando'],
            'crashed' => ['color' => 'danger', 'icon' => 'heroicon-m-x-circle', 'label' => 'Falhando'],
            'skipped' => ['color' => 'gray', 'icon' => 'heroicon-m-minus-circle', 'label' => 'Pulado'],
        ];
        $resolve = fn (string $status) => $statusMeta[$status] ?? ['color' => 'gray', 'icon' => 'heroicon-m-question-mark-circle', 'label' => $status];

        // Display formatter for arbitrary meta values (gettype avoids is_* guards).
        ...
        @else
            ...
            @if ($lastRanAt)
                <p @class([
                    ...
                    Última verificação: {{ $lastRanAt->format('d/m/Y H:i:s') }}
                    @if ($isStale) — desatualizado @endif
                </p>
            @endif
                                    ...
                                    <table class="w-full text-left">
                                        @foreach ($check->meta as $key => $value)
                                            <tr class="align-top">
                                                ...
                                                <td class="font-mono text-gray-700 dark:text-gray-200">
                                                    {{ $formatMeta($value) }}
                                                </td>
                                            </tr>
                                        @endforeach
